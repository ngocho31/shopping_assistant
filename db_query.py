...
class DBQuery:
    ...
    def get_db_results_for_slots(self, current_informs):
        ...
        for key, value in size_db_results_slots.items():
            if key == 'matching_all_constraints':
                continue
                # if value < db_results_slots['matching_all_constraints']:
                #     db_results_slots.update({'matching_all_constraints': value})
    ...
    def fill_inform_slot(self, inform_slot_to_fill, current_inform_slots, entity_list):
        ...
        # for key in self.size_slots:
        #     if key in current_informs:
        size_customer = ''
        if key == 'size_product':
            ...
            if size_db_results == -1:
                ...
                if size_values_dict:
                    ...
                    # size_customer = max(values_dict, key=values_dict.get)
                    ...
                    #     size_customer = list(values_dict.keys())
                    # else:
                    #     size_customer = list(values_dict.keys())[0]
            else:
                size_customer = 'no match available'
